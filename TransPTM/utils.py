...
from sklearn.metrics import roc_auc_score, average_precision_score, matthews_corrcoef, f1_score,recall_score
...
class TrainProcessor:
    ...
    def test(self, model, dataloader):
        ...
        metrics['sn'] = recall_score(y_pred=pred.round(), y_true=y, pos_label=1)
        metrics['sp'] = recall_score(y_pred=pred.round().round(), y_true=y, pos_label=0)
        metrics['auroc'] = roc_auc_score(y_true=y, y_score=pred)
        metrics['auprc'] = average_precision_score(y_true=y, y_score=pred)

        metrics['mcc'] = matthews_corrcoef(y_true=y, y_pred=pred.round())
        metrics['f1'] = f1_score(y_true=y, y_pred=pred.round())
